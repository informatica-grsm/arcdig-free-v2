...
public class AppMetaData extends SelectorComposer<Component> {

    public AppMetaData() {
    }
    List<Metadata> items = (List<Metadata>) new ArrayList<Metadata>();
    private Metadata SelectItem;
    ...
    public void dibujarMetadata() {
        items = new MetaDataDao().listarMetadata(filtro.getValue());
        ...
        for (Metadata item : items) {
            ...
            nombre.appendChild(new Label(item.getCampo()));
    ...
    public void select() {
        Set item = listMetadata.getSelectedItems();
    ...
    public void aceptar() {
        ...
        if (!"".equals(nombre.getValue())) {
            ...
            bean.setCampo(nombre.getValue().toUpperCase());
            bean.setEstado(estado.isChecked());
            Integer i = 0;
            if (editar) {
                bean.setId(SelectItem.getId());
                 i = new MetaDataDao().getRepetidoUpdate(bean);
            } else {
                 i = new MetaDataDao().getRepetido(bean);
            }
            if (i == 0) {
                if (editar) {
                    bean.setId(SelectItem.getId());
                    new MetaDataDao().update(bean);
                } else {
                    new MetaDataDao().insert(bean);
                }
                listMetadata.setVisible(true);
                formulario.setVisible(false);
                control.setVisible(false);
    ...
    public void editar() {
        if (SelectItem != null) {
            listMetadata.setVisible(false);
            formulario.setVisible(true);
            control.setVisible(true);
            nombre.setValue(SelectItem.getCampo());
    ...
    public void eliminar() {
        if (SelectItem != null) {
            Messagebox.show(
                    "Desea eliminar este Campo Extra?", "Dialogo de Confirmación ",
                    ...
                    new org.zkoss.zk.ui.event.EventListener() {
                        ...
                        public void onEvent(Event evt) throws Exception {
                            if (evt.getName().equals("onOK")) {
                                new MetaDataDao().delete(SelectItem);
                                dibujarMetadata();
                            }
                        }
                    });
